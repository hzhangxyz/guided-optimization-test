...
class BooleanMLP(nn.Module):
    def __init__(self, input_size=10):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(32, 1)  # Parameters: weight [2, 32], bias [2]
        )

    def forward(self, x):
        # Convert boolean to float, track operations if training
        return self.network(x.float())[...,0]
...
while True:
    ...
    for s in s_sample:
        ...
        wvec=model(num2bin(loc, input_size)).unsqueeze(1)
        ws=model(num2bin(torch.tensor(s), input_size))
    ...
    if cycle_count%output_interval==0:
        ...
        if loss_record is not None and abs(loss_record-loss_avg)<1e-7:
            break
        loss_record=loss_avg
